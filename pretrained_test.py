...
def test(model, loss_fn, test_loader, device):
    ...
    for imgs, labels in test_loader:
        ...
        argmaxOutput = torch.argmax(outputs['out'], dim=1)
...
def main():
    global n_epochs, batch_size, showGraphs

    # Get arguments
    argParser = argparse.ArgumentParser()
    argParser.add_argument('-m', metavar='model', type=str,
                           help='model to use [Solo (s), Response-Based (r), Feature-based (f)]', default='s') # Kept to prevent errors when adjusting command line args
    ...
    model.to(device)
    # summary(model, input_size=(batch_size, 3, 224, 224), col_names=("input_size", "output_size", "num_params"), depth=2)
